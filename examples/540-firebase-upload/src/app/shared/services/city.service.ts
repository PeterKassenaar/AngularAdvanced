...
export class CityService {
  ...
  sortCities(field: string, order: OrderByDirection = 'asc') {
    // We have to create a new collection, as the original collection is immutable.
    this.citiesCollection = this.afs.collection(environment.cities_endpoint, ref => {
      return ref.orderBy(field, order);
    });
    this.getCollection();
  }
}
